...
(define run-transition (λ (First-state event-seq STATE-TABLE )
                       (cond                                                                               ;It is a conditional statement for the code
                          ((null? event-seq) "false")                                                      ;If the event-seq is empty then it will return false or else the next state that I'll get will set
                         (else                                                                             ;in the first state that is the first argument in te function and then  call the function
                          (set! First-state (further-state First-state (first event-seq) STATE-TABLE))     ; using the value of the first state and print the sequence until
                          (run-transition First-state (rest event-seq) STATE-TABLE )                       ;the event sequence list becomes empty.
                          (sleep 0)
                          (println First-state)))))
;-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
;more sobs                                                                                                                                                        |
